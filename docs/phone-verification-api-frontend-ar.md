...
| `otp` | **يظهر مؤقتاً للتجربة** طالما خدمة الـ SMS غير مفعّلة بعد (أو في وضع debug). بمجرد تفعيل بوابة SMS (`VONAGE_SMS_ENABLED=true`) **لن يظهر** — الرمز يصل بالـ SMS فقط. لا تبنِ منطقاً يعتمد على وجوده في الإنتاج النهائي. |
...
2. **`otp` في الرد = للتجربة المؤقتة فقط** (لأن خدمة SMS لسا غير مفعّلة). أول ما تنفعّل بوابة الـ SMS بيختفي الرمز من الرد ويوصل بالـ SMS فقط. لا تبنِ منطقاً يعتمد على وجوده.
