...
## 34. Daily Report — تاب التقارير (Z-Report) — 2026-08-15

تابٌ رابع (`/dashboard/reports`) يولّد **تقريرًا مطبوعًا** بتخطيط عمودين على غرار Z-Report في أنظمة الكاشير: ملخص مبيعات + توزيع طرق الدفع + إجماليات + ملخص ضريبي + خصومات + مؤشرات تشغيلية + تفصيل خدمات + جدول موظفين + بطاقة تفصيل لكل موظف + جدول أداء يومي (للنطاق) + ملحق بكل المعاملات.

### 34.1 الفارق الجوهري عن تاب Statistics

| | Statistics (`StaffStats`) | Reports (`StaffReports`) |
|---|---|---|
| السؤال | "كيف كان **أداء** اليوم؟" | "كم **مالًا دخل** الصندوق؟" |
| أساس التاريخ | `appointment_date` | **تاريخ التحصيل الفعلي** |
| وحدة الحساب | الموعد (`appointment`) | **الفاتورة** (= إيصال = معاملة) |
| النطاق | يوم واحد | يوم أو نطاق تواريخ |
| الإخراج | شاشة حيّة | مستند A4 مستقل للطباعة |
| الصلاحية | `view_stats` (admin + provider) | `view_reports` (**إداري فقط**) |

الرقمان قد **لا يتطابقان عمدًا**: حجز الاثنين المدفوع الثلاثاء يظهر في أداء الاثنين وفي نقد الثلاثاء.

### 34.2 سلسلة حلّ تاريخ التحصيل (الأهم في الميزة)

سجلات `payments` تُكتب **فقط** من `InvoiceFinalizationService` (مسار الدفع من الداشبورد). الحجوزات النقدية القادمة من الـ API تُعلَّم مدفوعة عند الإنشاء **بلا سجل دفع**. لذا `resolveCollectedAt()` يتدرّج:

1. `payments.created_at` لأول دفعة ناجحة — المصدر الموثوق.
2. `invoice_data.finalized_at` — شاهد ثانٍ يكتبه نفس المسار.
3. `invoice.updated_at` — تقريب للفواتير المُنهاة من مسارات أخرى.
4. `appointment_date` — الملاذ الأخير (حالة API النقدية).

`resolveMethod()` يتبع نفس ترتيب الثقة لتحديد `cash|card|online`.

**الاستعلام على مرحلتين إجباريًا**: لا يمكن لـ SQL أن يفلتر على تاريخ محلول من ثلاثة مصادر، لذا يُجلب **superset** بـ OR ثلاثي (كل فرع يقابل حلقة من السلسلة) ثم يُفلتر بدقة في PHP.

### 34.3 قرارات دقّة حرجة

1. **الفاتورة هي وحدة الحساب** — تعيش دائمًا على الأب في المجموعة المرتبطة، و`total_amount` فيها هو المستحق للمجموعة كلها ⇒ عدّ الفواتير لا يضاعف أبًا وابنًا أبدًا. راجع [[payment-discount-unified]].
2. **توزيع الخصم تناسبيًا** (`coverageOf()`): الخصم يعيش على الفاتورة وحدها، لكن الفاتورة المجمّعة قد تغطي عدة مزوّدين. يُوزَّع بحصة كل موعد من الإجمالي، **والصف الأخير يمتصّ باقي التقريب**، فيتحقّق: Σ(إيراد الموظفين) == Σ(إجماليات الفواتير) بالضبط — وهذا ما يجعل جدول الموظفين يطابق ملخص المبيعات (يحرسه اختبار).
3. **`booking_source` يُقارَن كـ enum لا كنص.** `DashboardStatsService::sourceOf()` يقارنه بسلسلة `'online'` بينما العمود مُحوَّل إلى `BookingSource` ⇒ **تاب Statistics يقرأ كل الحجوزات كـ "استقبال"**. الخطأ ما زال قائمًا هناك (خارج نطاق هذا العمل) — لا تنسخه.
4. **`whereDate` على الطرفين** لأعمدة `appointment_date` / `work_date`: كلاهما datetime، و`whereBetween` بحدّ أعلى `Y-m-d` يُقارَن كمنتصف الليل فيُسقط اليوم الأخير كاملًا.
5. **وقت العمل من الحضور الفعلي** (`provider_attendances`) لا من الدوام المخطط: `minutes` تجمع الجلسات **المغلقة فقط** (جلسة منسية لا تضخّم الرقم) و`open_sessions` تكشف ذلك بدلًا منها. بلا حضور ⇒ `null` ⇒ "لم يسجّل حضورًا".
6. **الملغى / لم يحضر / المتبقي للتحصيل** تُحسب بـ `appointment_date` اضطرارًا (لا تاريخ تحصيل لها)، والواجهة تُصرّح بذلك في سطر تحت القسم.
7. **إعدادات الشركة الفارغة نصوص لا nulls** ⇒ `SettingsService::get($k, $default)` لا يرجع للافتراضي أبدًا؛ لذا `setting()` يعامل الفراغ كغياب.
8. **الموظفون بلا إيراد** يظهرون في جدول الملخص بأصفار (ليرى المالك من كان خاملًا) لكن **بلا بطاقة تفصيل** (بطاقة كلها شرطات = ضجيج).

### 34.4 الملفات

| الملف | الدور |
|-------|-------|
| `app/Services/DailyReportService.php` | **كل المنطق**: `build($from,$to,$providerIds)` + سلسلة حلّ التاريخ/الطريقة + التوزيع التناسبي + كل الأقسام |
| `app/Livewire/StaffReports.php` | مكوّن نحيف: حالة الفلاتر + presets + معاينة حيّة + بناء رابط التقرير. **لا يُمرّر بيانات للمستند** |
| `resources/views/livewire/staff-reports.blade.php` | شاشة الفلاتر + المعاينة |
| `app/Http/Controllers/DailyReportController.php` | المستند المطبوع؛ `abort_unless(view_reports)` + سقف 366 يومًا + عكس النطاق المقلوب |
| `resources/views/reports/daily-report.blade.php` | مستند A4 مستقل بـ `<html>` وCSS خاص (نمط `AppointmentPrintController`) |
| `lang/{en,ar,de}/z_report.php` | ~110 مفتاح لكل لغة |
| `database/migrations/2026_08_15_000001_add_paymentable_index_to_payments_table.php` | فهرس `(paymentable_type, paymentable_id)` + `created_at` على `payments` |
| `tests/Feature/DailyReportTest.php` | 14 اختبارًا |

### 34.5 ملاحظات للمطورين

- **الرابط هو التقرير**: المستند يعيد اشتقاق كل شيء من `?from=&to=&providers[]=`، فلا يعتمد على حالة المكوّن ⇒ قابل للحفظ في المفضلة وإرساله للمحاسب وإعادة طباعته لاحقًا بنفس الأرقام.
- **CSS خاص لا Tailwind** في المستند: صفحة مستقلة موجَّهة للطباعة، وTailwind purge لن يضمن أصنافها.
- **`.ltr` لعزل الاتجاه**: نطاقات التواريخ/الأوقات (`12/08 – 15/08`، `09:07 – 17:22`) يعكسها خوارزم bidi في العربية فتُقرأ مقلوبة. كل نطاق ملفوف بـ `unicode-bidi: isolate`.
- **الدونات SVG خالص** (`stroke-dasharray` + offset تراكمي) — لا مكتبة رسوم، يعمل في الطباعة.
- **`table-layout: fixed` + رؤوس تلتف** في جدول الموظفين: سبعة أعمدة مالية في 61% من العرض تُخرج العمود الأخير خارج الورقة بدونها.
- **بوّابة مزدوجة**: التاب مخفي بلا `view_reports`، و`StaffReports::mount()/render()` + الكنترولر يعملون `abort_unless(...,403)`.
