...
1. نفّذ `finalizeAppointmentPayment()` مرة أولى.
2. حاول تنفيذ العملية نفسها مرة ثانية لنفس الموعد.
...
### FIN-07 التحقق من السعر الخاص الكامل — لا دفعات جزئية

**الهدف**

- التأكد من أن المبلغ الأقل يُعامل كسعر خاص/خصم مع دفع كامل، حسب قرار MON-05.

**خطوات التنفيذ**

1. جهّز موعداً بخدمات مجموعها `100.00` وفاتورة DRAFT.
2. نفذ `finalizeAppointmentPayment()` بمبلغ `80.00` وطريقة cash/card فعالة.
3. راجع Invoice وPayment وappointment.
4. حاول دفعاً ثانياً أو مبلغاً `0` أو أكبر من `100.00`.

**النتيجة المتوقعة**

- `discount_amount = 20.00` و`invoice.status = PAID`.
- Payment واحد من `type=full` ومبلغه `80.00` وله `payment_method_id`.
- `subtotal + tax_amount = total_amount = payment.amount`.
- لا يبقى رصيد دين ولا تُقبل دفعة ثانية.
- الصفر والدفع الزائد يُرفضان بلا كتابة جزئية.
